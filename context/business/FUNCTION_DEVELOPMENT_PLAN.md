...
#### 前端对齐
- ⚠️ **BranchRule 前端升级**
  - 现状：使用旧 API 模型（ALLOW/BLOCK）
  - 目标：对接新 API（TEMPLATE/REGEX），支持 scope/启用/禁用/测试
  - **优先级：P1**

- ⚠️ **Version Ops Dashboard 对接**
  - 现状：页面存在但数据硬编码
...
- ⚠️ **分组 code 自动生成**
  ...
  - 现状：接口仍要求提交 code（实现缺口）

- ⚠️ **groupCode 关联到发布窗口/迭代/仓库**
  - 目标口径：创建/修改时必选末端分组并提交 groupCode
  - 现状：相关请求体未体现 groupCode（实现缺口）
...
### 6.2 待解决的技术债务
1. **BranchRule 前端升级未完成**
   - 前端仍使用旧模型（ALLOW/BLOCK）
   - 影响：无法使用 scope/启用/禁用/测试能力

2. **Version Ops Dashboard 未对接**
   - 页面数据硬编码
   - 影响：无法展示真实运行数据
