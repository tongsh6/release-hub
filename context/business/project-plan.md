...
1. 分组 code 自动生成与末端节点校验（P1，1-2 天）
2. groupCode 关联到发布窗口/迭代/仓库（P1，1-2 天）
3. 分支自动化与归档规则对齐（P1，2-3 天）
4. BranchRule 前端升级（P1，1-2 天）
5. Version Ops Dashboard 对接（P1，2-3 天）
...
| 阶段 | 内容 |
|------|------|
| **Phase 1** | 前端对齐：BranchRule 新 API 对接、Version Ops Dashboard |
| **Phase 2** | 体验增强：迭代/发布窗口日历视图（统计视角）、冲突检测可视化 |
| **Phase 3** | 通知集成：飞书/钉钉/邮件 |
| **Phase 4** | 企业级特性：RBAC 权限、多租户 |
