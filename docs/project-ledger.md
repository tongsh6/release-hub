...
| 场景化验收矩阵基线复验 | `bash scripts/acceptance/run-acceptance.sh` | `docs/reports/scenario-acceptance-matrix.md` | **88 PASS / 0 FAIL / 0 SKIP** |
...
| 优先级 | 事项 | 原因 | 验收标准 |
|---|---|---|---|
| P1 | SA-010/SA-011 发布计划与风险详情 | attach 和冲突阻断已有强证据，发布计划、冲突严重级别、建议处理方式以及 `MERGE_CONFLICT`/`CROSS_REPO_VERSION_MISMATCH` 类型分布和详情已补前端观察；`MERGE_CONFLICT` 已补真实 GitLab 后端强证据 | 补更多真实冲突类型后端/GitLab 强证据和部分失败重试 |
...
| 证据 | 路径 | 说明 |
|---|---|---|
| 最末验收报告 | `docs/reports/scenario-acceptance-matrix.md` | 2026-05-17 SA-011 MERGE_CONFLICT 后端/GitLab 强证据补强；真实 GitLab 验收：88 PASS / 0 FAIL / 0 SKIP；当前推进队列在第七节 |
...
| 验收脚本 | `scripts/acceptance/run-acceptance.sh` | v3.8，含服务生命周期、`--hold-services`、SA-011 MERGE_CONFLICT 强证据、SA-012 分支名不合规强证据、SA-013 干净黄金路径、SA-014 GitLab commit 校验 |
