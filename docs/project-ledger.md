...
| 优先级 | 事项 | 原因 | 验收标准 |
|---|---|---|---|
| P1 | SA-010 发布计划与解除挂载收口 | attach、同分组挂载约束和冲突阻断已有强证据，发布计划、挂载弹窗非同分组禁选、解除挂载入口、解除挂载 Slice-1 UI E2E、解除挂载真实 GitLab release 分支归档、发布后计划变更锁定、冲突严重级别、建议处理方式以及 SA-011 六类风险详情均已补前端观察；`MERGE_CONFLICT`/`CROSS_REPO_VERSION_MISMATCH`/`REPO_AHEAD`/`SYSTEM_AHEAD`/`GIT_PERMISSION_DENIED`/`GIT_UNAVAILABLE` 已补真实 GitLab 后端强证据；Run 详情失败项重试前端入口已补 | 后续保持回归 |
