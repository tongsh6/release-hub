...
| 优先级 | 事项 | 原因 | 验收标准 |
|---|---|---|---|
| P1 | SA-006 分支规则真实端到端证据收口 | 分支规则真实页面管理旅程、作用域、核心分支链路 scoped compliance 和 archive 分支统计治理已补；仍缺“规则配置 → 分支创建被规则约束”的真实 GitLab 端到端证据 | 用真实后端/真实 GitLab 证明最具体规则约束 feature/release 分支创建，并同步矩阵与任务记录 |
