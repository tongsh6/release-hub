...
| 事项 | 当前状态 | 下一步 | 验收标准 |
|---|---|---|---|
| 场景矩阵驱动推进 | 2026-05-21 已补 SA-016 CI pipeline 触发状态：`CI_TRIGGERED` 保留 pipeline id，`CI_NOT_CONFIGURED` 不伪装成 `SUCCESS`；同日已补 SA-012 `REPO_AHEAD` 接受仓库版本解决路径：窗口冲突面板提交 `USE_REPO`，后端更新系统版本并标记来源 `REPO`；同日已补 SA-014 版本更新失败重试：Run 详情只提交版本更新失败项，后端只重放 `VERSION_UPDATE_FAILED` 并保留 retry 来源；同日已补 SA-005 删除保护扩展：仓库被迭代引用时拒绝删除，分组有子分组或被仓库/迭代/发布窗口引用时拒绝删除，仓库列表、分组列表和分组详情展示明确阻断提示；同日已补 SA-005 仓库组织筛选、版本解析失败修复引导和 Clone URL 纳管保护；同日已补 SA-009 已挂窗口后迭代仓库集合锁定和同分组仓库选择与写入保护；2026-05-20 已补 SA-014 批量版本更新多仓部分失败、Maven 多模块/Gradle 真实写回、批量版本更新前端入口；同日已补 SA-015 Run 详情失败项重试前端闭环；同日已补 SA-010 发布后发布计划变更锁定、同分组挂载范围约束、SA-003 资源创建叶子分组前端断言、SA-010 解除挂载真实 GitLab 分支归档证据、SA-011 Git 访问异常真实 GitLab/Playwright 证据；SA-015 已补分组筛选复核、窗口详情冲突证据复核、Run 详情部分失败复核和真实部分失败重试后端/GitLab 证据；SA-016 已补发布窗口报告导出 API 和详情页 CSV 导出入口；SA-011 已补六类风险后端/GitLab 强证据和前端详情观察；SA-012 已补 feature 缺失、release 分支已存在、分支名不合规强证据 | 按 `docs/execution-roadmap.md` 当前 HEAD 继续 SA-008 发布窗口列表/日历前端证据扩展 | 每个场景都同时具备前端用户旅程、后端业务约束、真实 GitLab/数据证据，并在矩阵中更新状态 |
...
| 优先级 | 事项 | 原因 | 验收标准 |
|---|---|---|---|
| P1 | SA-008 发布窗口列表/日历前端证据扩展 | 发布窗口创建和详情主线已闭环；列表/日历仍缺组织路径、筛选和冻结限制证据 | 补列表/日历前端可观察性和测试证据 |
