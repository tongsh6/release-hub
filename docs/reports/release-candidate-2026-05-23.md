...
受控处置执行审计设计和最小处置 case 已形成，见 `docs/openspec/changes/update-data-quality-disposition-audit/`。下一阶段若要真实处理历史风险，必须先从页面场景验收固化创建、查看和状态记录证据，确认处置 case 仍只记录审计状态，不执行清理。
...
| 数据质量治理 | 已具备受控处置策略 | dry-run 188 条待复核动作，人工复核入口拒绝直接执行，复核结果展示处置等级、允许动作、回滚边界和审计记录，`--execute` 继续拒绝 |
...
- dry-run 动作已转为可筛选、可分派、可审计的人工复核队列，并具备处置等级、允许动作、失败回滚边界、审计记录口径和处置 case 状态记录。
- 下一阶段必须补处置 case 页面场景验收与证据归档，不能把复核队列直接升级为自动清理入口。
...
2. SA-002 数据质量复核队列、命名空间、数据源口径、人工复核处置策略、受控处置执行审计设计和最小处置 case 已完成；下一开发 HEAD 转向数据质量处置 case 场景验收与证据归档。
