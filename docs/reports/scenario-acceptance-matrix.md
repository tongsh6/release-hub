...
| SA-011 | 测试人员在窗口详情页触发/查看风险扫描 | 冲突总数、类型分布、阻塞发布 | 冲突与 GitLab 分支/版本状态可对应 | 严重级别和建议处理方式已有最小前端观察；更多真实冲突类型详情仍不足 |
| SA-012 | 技术负责人在冲突详情中执行解决动作 | `USE_SYSTEM` 等解决动作更新记录，重扫清零 | 必要时写回仓库或保留处理证据 | P0 版本冲突 `USE_SYSTEM` 已闭环；分支名不合规、release 分支已存在外部处理路径已有前端观察；更多冲突类型解决路径为 P1/P2 |
...
| SA-015 | 测试人员在 Run/窗口详情复核执行证据 | Run 列表、Run 详情、窗口详情返回完整状态；UI 触发失败版本更新后可按窗口、状态和分组筛选并复核失败 Run | RunItem/RunStep 可追溯到窗口、迭代、仓库和失败 POM 路径；Run 分页接口支持按发布窗口分组过滤 | P0 已闭环；冲突详情和部分失败复核旅程为 P1 |
...
| 优先级 | 场景 | 当前判断 | 下一步验收焦点 |
|---|---|---|---|
| P1 | SA-012 更多冲突解决路径 | 版本冲突 `USE_SYSTEM` 已闭环；feature 缺失发布计划观察、分支名不合规、release 分支已存在外部处理路径已有前端观察 | 补 feature 缺失对应后端/GitLab 强证据 |
| P1 | SA-010/SA-011 发布计划与风险详情 | attach、真实 release 分支、冲突阻断已有后端/GitLab 证据；发布计划、冲突严重级别、建议处理方式以及 `MERGE_CONFLICT`/`CROSS_REPO_VERSION_MISMATCH` 类型分布和详情已补前端观察 | 补更多真实冲突类型后端/GitLab 强证据和部分失败重试 |
| P1 | SA-015 复核扩展 | P0 已能由 UI 生成失败 Run，并按窗口、分组和失败状态复核失败步骤 | 补冲突详情和部分失败复核旅程 |
...
- SA-012 feature 缺失已具备前端可观察证据；后续仍需补真实 GitLab/后端强证据，确认缺失分支会阻断或显式报告。
...
- 后续仍需补 feature 缺失等路径，以及 release 分支已存在路径对应的后端/GitLab 强证据。
...
- 后续仍需补 release 分支已存在、feature 缺失等处理路径，以及对应后端/GitLab 强证据。
