...
| SA-012 | 技术负责人在冲突详情中执行解决动作 | `USE_SYSTEM` 等解决动作更新记录，重扫清零 | 必要时写回仓库或保留处理证据 | P0 版本冲突 `USE_SYSTEM` 已闭环；分支名不合规外部处理路径已有前端观察；更多冲突类型解决路径为 P1/P2 |
...
- release 分支已存在、feature 缺失等解决路径为 P1。
...
| 优先级 | 场景 | 当前判断 | 下一步验收焦点 |
|---|---|---|---|
| P1 | SA-012 更多冲突解决路径 | 版本冲突 `USE_SYSTEM` 已闭环；分支名不合规外部处理路径已有前端观察 | 扩展 release 分支已存在、feature 缺失等处理路径 |
