...
| SA-011 | Risk & Execution | 测试人员 | 检查冲突与发布风险 | 冲突扫描、类型分布、阻塞发布、解决后重扫清零；`MERGE_CONFLICT` 具备真实 GitLab 强证据 | 部分覆盖 |
...
| SA-011 | 测试人员在窗口详情页触发/查看风险扫描 | 冲突总数、类型分布、阻塞发布；`MERGE_CONFLICT` 已有后端冲突扫描证据 | 冲突与 GitLab 分支/版本状态可对应；`MERGE_CONFLICT` 已有真实 feature/release 分支和冲突提交证据 | 严重级别和建议处理方式已有最小前端观察；更多真实冲突类型详情仍不足 |
...
- `MERGE_CONFLICT` 已具备真实 GitLab 强证据；更多真实冲突类型的详情字段复核为 P1。
...
| 优先级 | 场景 | 当前判断 | 下一步验收焦点 |
|---|---|---|---|
| P1 | SA-010/SA-011 发布计划与风险详情 | attach、真实 release 分支、冲突阻断已有后端/GitLab 证据；发布计划、冲突严重级别、建议处理方式以及 `MERGE_CONFLICT`/`CROSS_REPO_VERSION_MISMATCH` 类型分布和详情已补前端观察；`MERGE_CONFLICT` 已补真实 GitLab 后端强证据 | 补更多真实冲突类型后端/GitLab 强证据和部分失败重试 |
