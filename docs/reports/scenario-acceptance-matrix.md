...
- 前端风险展示已补最小严重级别、建议处理方式、`MERGE_CONFLICT` 类型分布/分支详情、`CROSS_REPO_VERSION_MISMATCH` 版本差异详情，以及 `REPO_AHEAD` / `SYSTEM_AHEAD` 版本领先详情；后续补 GitLab 不可达/权限失败等外部系统异常路径。
...
| 优先级 | 场景 | 当前判断 | 下一步验收焦点 |
|---|---|---|---|
| P1 | SA-010/SA-011 发布计划与风险详情 | attach、真实 release 分支、冲突阻断已有后端/GitLab 证据；发布计划、解除挂载入口与解除挂载 Slice-1 UI E2E、冲突严重级别、建议处理方式以及 `MERGE_CONFLICT`/`CROSS_REPO_VERSION_MISMATCH`/`REPO_AHEAD`/`SYSTEM_AHEAD` 类型分布和详情已补前端观察；上述四类冲突均已补真实 GitLab 后端强证据 | 后续补 GitLab 不可达/权限失败类风险、带仓库解除挂载的真实 GitLab 分支归档复核与更完整发布计划限制 |
