...
- 更多冲突类型的详情字段与前端类型分布复核为 P1。
- GitLab 不可达/权限失败类冲突为 P1。
- 前端风险展示已补最小严重级别和建议处理方式，后续补更多真实冲突类型观察路径。
...
| 优先级 | 场景 | 当前判断 | 下一步验收焦点 |
|---|---|---|---|
| P1 | SA-012 更多冲突解决路径 | 版本冲突 `USE_SYSTEM` 已闭环；分支名不合规、release 分支已存在外部处理路径已有前端观察 | 扩展 feature 缺失等处理路径，并补对应后端/GitLab 强证据 |
| P1 | SA-010/SA-011 发布计划与风险详情 | attach、真实 release 分支、冲突阻断已有后端/GitLab 证据；发布计划、冲突严重级别和建议处理方式已补前端观察 | 补更多真实冲突类型详情、类型分布复核和部分失败重试 |
