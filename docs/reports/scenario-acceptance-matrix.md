...
|---|---|---|---|---|
| SA-003 | 管理员在分组页面创建多层分组并查看树 | Group API、非叶子资源挂载拒绝；分组父级变更受后端权威约束保护，只有未挂资源且无子分组的空叶子分组可移动；目标父分组若已挂资源则拒绝成为新上级 | 仓库/迭代/窗口 groupCode 均落在叶子分组；受控移动不会把已挂资源分组变成非叶子分组 | 前端已稳定断言仓库、迭代、发布窗口创建入口只能选择叶子分组；关联资源删除保护和受控空叶子分组移动拒绝提示已补；后续补页面级父级选择体验和外部 Playwright 旅程复核 |
...
- 页面级父级选择器仍是 P1：当前编辑入口仍以父级编码输入为主，后续应以组织树选择器减少误输并更直接表达可选目标。
...
| 优先级 | 场景 | 当前判断 | 下一步验收焦点 |
|---|---|---|---|
| P1 | SA-003 组织资源移动治理 | 三层分组、叶子分组资源挂载、删除保护和受控空叶子分组移动已有证据；当前仍保留页面级父级选择体验和外部旅程复核缺口 | 分组移动/资源迁移产品边界已收口为受控空叶子分组移动；后续补前端树选择体验，继续保证资源移动不会破坏叶子分组归属、发布范围追溯和既有关联保护 |
