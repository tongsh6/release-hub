...
| SA-014 | 技术负责人在版本操作入口执行版本更新 | 版本更新 Run COMPLETED/SUCCESS，失败原因可见；多仓窗口可从版本更新弹窗提交批量版本更新请求 | `pom.xml` 在 release 分支真实 commit；批量请求按仓库生成 repoPath 并调用既有后端批量端点 | P0 Maven 单模块已闭环；批量版本更新前端入口已补；多模块和 Gradle 真实写回仍为 Phase 2 |
...
| P2 | SA-014 版本更新扩展 | Maven 单模块已闭环；批量版本更新前端入口和请求契约已补 | Maven 多模块、Gradle 真实写回、失败重试和多仓部分失败 |
...
- SA-014 多仓版本更新已具备前端入口和请求契约；真实 Maven 多模块、Gradle 写回和多仓部分失败仍保留为后续真实 GitLab 验收扩展。
