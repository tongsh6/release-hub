...
| SA-005 | 管理员在仓库页纳管分组仓库并查看详情 | 仓库创建校验、叶子分组归属、重复/错误 URL 校验；详情页/抽屉展示组织路径和版本解析状态 | 真实 GitLab cloneUrl、默认分支、token 安全审计；初始版本来源 `versionSource` 可复核 | 重复/错误 URL、删除保护和版本解析失败更多引导为 P1/P2 |
