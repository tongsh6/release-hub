...
| SA-010 | 发布经理在窗口详情页挂载迭代并查看发布计划 | attach/detach 细粒度结果、状态流转、冲突阻断；解除挂载已有前端详情页入口与后端约束 | release 分支真实创建，WindowIteration 状态一致；部分失败重试已有后端/GitLab 证据 | 发布计划已有最小前端观察；解除挂载已补 Vitest 证据，后续保留真实 GitLab/UI E2E 复核 |
...
- 同分组范围约束、更完整发布计划限制和解除挂载真实 GitLab/UI E2E 复核为 P1。
- 部分成功/部分失败和失败重试需要补强。
...
| 优先级 | 场景 | 当前判断 | 下一步验收焦点 |
|---|---|---|---|
| P1 | SA-010/SA-011 发布计划与风险详情 | attach、真实 release 分支、冲突阻断已有后端/GitLab 证据；发布计划、解除挂载入口、冲突严重级别、建议处理方式以及 `MERGE_CONFLICT`/`CROSS_REPO_VERSION_MISMATCH`/`REPO_AHEAD`/`SYSTEM_AHEAD` 类型分布和详情已补前端观察；上述四类冲突均已补真实 GitLab 后端强证据 | 后续补 GitLab 不可达/权限失败类风险、解除挂载真实 GitLab/UI E2E 复核与更完整发布计划限制 |
