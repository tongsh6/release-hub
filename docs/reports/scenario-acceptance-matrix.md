...
| SA-011 | Risk & Execution | 测试人员 | 检查冲突与发布风险 | 冲突扫描、类型分布、阻塞发布、解决后重扫清零；`MERGE_CONFLICT`、`CROSS_REPO_VERSION_MISMATCH` 具备真实 GitLab 强证据 | 部分覆盖 |
...
| SA-010 | 发布经理在窗口详情页挂载迭代并查看发布计划 | attach 细粒度结果、状态流转、冲突阻断 | release 分支真实创建，WindowIteration 状态一致；部分失败重试已有后端/GitLab 证据 | 发布计划已有最小前端观察；解除挂载不足 |
| SA-011 | 测试人员在窗口详情页触发/查看风险扫描 | 冲突总数、类型分布、阻塞发布；`MERGE_CONFLICT`、`CROSS_REPO_VERSION_MISMATCH` 已有后端冲突扫描证据 | 冲突与 GitLab 分支/版本状态可对应；`MERGE_CONFLICT` 已有真实 feature/release 分支和冲突提交证据；`CROSS_REPO_VERSION_MISMATCH` 已有真实 feature/release 分支、两仓 targetVersion 差异和冲突扫描证据 | 严重级别和建议处理方式已有最小前端观察；更多真实冲突类型详情仍不足 |
...
| SA-015 | 测试人员在 Run/窗口详情复核执行证据 | Run 列表、Run 详情、窗口详情返回完整状态；UI 触发失败版本更新后可按窗口、状态和分组筛选并复核失败 Run；窗口详情可复核冲突类型分布、分支/版本详情和建议处理方式；Run 详情可复核一个 Run 内成功项与失败项并存；后端/GitLab 证据可复核真实部分失败重试只选择失败项 | RunItem/RunStep 可追溯到窗口、迭代、仓库和失败 POM 路径；Run 分页接口支持按发布窗口分组过滤；retry 新 Run 保留选中失败项且不重复执行成功项 | P0 已闭环；部分失败重试后端/GitLab 强证据已补，发布报告导出仍为 P2 |
| SA-016 | 发布经理在窗口详情页关闭窗口并查看收尾结果 | CLOSED 状态、关闭后关键操作禁止、重复关闭幂等 | tag、merge、归档和收尾 Run 可追踪 | P0 已闭环；真实部分失败重试后端/GitLab 证据已补，发布报告导出仍为 P2 |
...
- `MERGE_CONFLICT`、`CROSS_REPO_VERSION_MISMATCH` 已具备真实 GitLab 强证据；更多真实冲突类型的详情字段复核为 P1。
- GitLab 不可达/权限失败类冲突为 P1。
- 前端风险展示已补最小严重级别、建议处理方式、`MERGE_CONFLICT` 类型分布/分支详情和 `CROSS_REPO_VERSION_MISMATCH` 版本差异详情，后续补更多真实冲突类型观察路径。
...
- 当前主线可转向 SA-010/SA-011 更多真实冲突类型后端/GitLab 强证据和报告导出类扩展。
...
- `run-acceptance.sh` 5.8 已补真实 GitLab 部分失败重试证据：同一 attach Run 内构造一个 `MERGED` 仓库项和一个 `MERGE_BLOCKED` 仓库项，再调用 retry API 验证新 Run 只包含选中的失败项。
...
- `run-acceptance.sh` 5.8 已补真实部分失败重试后端/GitLab 强证据：部分成功/部分阻塞 attach Run 可选择失败项重试，成功项不会被重复执行。
...
| 优先级 | 场景 | 当前判断 | 下一步验收焦点 |
|---|---|---|---|
| P1 | SA-010/SA-011 发布计划与风险详情 | attach、真实 release 分支、冲突阻断已有后端/GitLab 证据；发布计划、冲突严重级别、建议处理方式以及 `MERGE_CONFLICT`/`CROSS_REPO_VERSION_MISMATCH` 类型分布和详情已补前端观察；`MERGE_CONFLICT`、`CROSS_REPO_VERSION_MISMATCH` 已补真实 GitLab 后端强证据 | 补更多真实冲突类型后端/GitLab 强证据 |
| P1 | SA-015 复核扩展 | P0 已能由 UI 生成失败 Run，并按窗口、分组和失败状态复核失败步骤；窗口详情冲突证据复核、Run 详情部分失败复核和真实部分失败重试后端/GitLab 证据已补 | 后续转入报告导出类扩展 |
| P1 | SA-016 收尾扩展 | P0 已闭环，重复关闭幂等和真实部分失败重试已纳入验收脚本 | 补发布报告导出 |
...
- 真实部分失败重试已从前端/Run 详情观察扩展为后端/GitLab 强证据；后续 SA-015/SA-016 P1 队列保留发布报告导出。
