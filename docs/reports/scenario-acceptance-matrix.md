...
本矩阵不以现有实现为边界。每个场景先定义完整业务目标，再标注当前覆盖和缺口。

覆盖状态定义：

| 状态 | 含义 |
|---|---|
| 已覆盖 | 当前实现和自动化基本满足 P0 验收焦点 |
| 部分覆盖 | 当前已有实现或测试，但缺少关键业务约束、真实 GitLab 证据或前端可观察性 |
| 缺口较大 | 当前能力、自动化或前端体验与完整蓝图差距明显 |

验证层级：

| 层级 | 主入口 |
|---|---|
| 真实 GitLab 验收 | `scripts/acceptance/run-acceptance.sh` |
| 后端 E2E/API | `backend/releasehub-bootstrap/src/test/java/io/releasehub/bootstrap` |
| 前端 Playwright | `frontend/e2e/tests` |
| 验收报告 | `docs/reports/acceptance-*.md` |
...
- `run-acceptance.sh` 仍是单层“验收分组”。

缺口：

- 非叶子资源挂载约束需要跨仓库、迭代、窗口统一验证。
...
3. 三层分组：把脚本中的单层验收分组升级为客户/业务线/品牌。
