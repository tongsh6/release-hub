...
|---|---|---|---|---|
| SA-003 | 管理员在分组页面创建多层分组并查看树 | Group API、非叶子资源挂载拒绝；分组父级变更受后端权威约束保护，只有未挂资源且无子分组的空叶子分组可移动；目标父分组若已挂资源则拒绝成为新上级 | 仓库/迭代/窗口 groupCode 均落在叶子分组；受控移动不会把已挂资源分组变成非叶子分组 | 前端已稳定断言仓库、迭代、发布窗口创建入口只能选择叶子分组；关联资源删除保护、受控空叶子分组移动拒绝提示、父级组织树选择器和外部 Playwright 移动旅程已补；后续仅保留 code 自动生成证据复核 |
...
- code 自动生成为 P1/P2，按实现成熟度评估并补当前测试证据。
...
| 优先级 | 场景 | 当前判断 | 下一步验收焦点 |
|---|---|---|---|
| P1 | SA-003 组织资源移动治理 | 三层分组、叶子分组资源挂载、删除保护、受控空叶子分组移动、页面级父级树选择器和外部移动旅程已有证据；当前仅保留 code 自动生成证据复核缺口 | 补齐 code 自动生成当前测试证据后，SA-003 可转入后续保持回归 |
...
- SA-003 仍保留 code 自动生成当前测试证据复核；资源移动治理和页面级父级选择体验已收口。
