...
### 设计文档先行（NON-NEGOTIABLE）

任何功能实现、架构变更、Bug 修复（非单行修复），必须在编码前完成设计文档：

- **新功能 / 破坏性变更**：走 `requirements/`（需求文档）→ `openspec/`（规格提案）→ `tasks/plans/`（目标蓝图 + DAG + 垂直切片）
- **现有功能增强**：在 `tasks/plans/` 中创建目标蓝图，包含 DAG 和切片拆分
- **文档输出物**：需求文档（用户故事、验收标准）→ 规格（Purpose + Requirements + Scenarios）→ 目标蓝图（完整范围、非目标、架构形态、风险）→ DAG（依赖关系、并行可能性）→ 垂直切片（端到端路径、独立验收标准）

**蓝图未完成、未经评审确认，不允许写任何实现代码。**

`tasks/CONSTRAINTS.md` 第 1 条约束即为本原则的执行细则。
