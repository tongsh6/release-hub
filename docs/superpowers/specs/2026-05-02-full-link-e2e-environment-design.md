...
使用 **Docker Desktop（desktop-linux context）**，可通过 Dashboard GUI 查看容器状态、日志和数据卷。PostgreSQL 和 GitLab 常驻本机，数据持久化。每次测试只启动 backend + frontend，秒级开始。

```bash
# 切换到 Docker Desktop context（一次性）
docker context use desktop-linux
```

```
本机常驻（手动启动一次，一直运行）：
├── PostgreSQL (localhost:5433)  ← docker compose -f docs/docker-compose.yml up -d
└── GitLab CE  (localhost:9080)  ← docker compose -f docker-compose.gitlab.yml up -d
...
使用 **colima context**，轻量无 GUI，适合 CI 自动化。所有服务从零启动，跑完自动销毁，不留痕迹。
