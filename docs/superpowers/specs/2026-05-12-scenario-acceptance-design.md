...
在 `docs/reports/scenario-acceptance-matrix.md` 中为 SA-003 到 SA-016 增加证据拆分：

- 用户旅程入口：用户从哪个页面完成操作或复核。
- 后端业务证据：哪些约束、状态流转或错误阻断必须成立。
- GitLab/数据证据：哪些分支、commit、RunItem、RunStep 或审计结果证明结果可信。
- 当前主要缺口：缺 UI、缺业务约束、缺 GitLab 证据或缺异常恢复时必须明确标出。
