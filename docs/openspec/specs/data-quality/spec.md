...
### Requirement: 本地开发库可重复清理入口

系统 SHALL 提供独立于 SA-002 复核队列的本地开发库清理脚本，用于开发阶段反复清理验收、联调和历史脏数据。

#### Scenario: dry-run 预览开发库清理范围

- **WHEN** 开发者运行本地开发库清理脚本且未显式执行
- **THEN** 系统生成清理报告，列出目标 schema、候选表、保留表和执行前行数
- **AND** 默认不得修改数据库
- **AND** 报告 SHALL 明确该脚本只适用于本地 `releasehub-postgres` / `release_hub` 开发库

#### Scenario: 执行开发库业务数据清理

- **WHEN** 开发者使用显式执行参数运行本地开发库清理脚本
- **THEN** 系统 SHALL 清理本地开发库业务表数据，并保留 schema、Flyway 元数据、用户账号和系统设置
- **AND** 系统 SHALL 同时覆盖当前应用 schema 和历史遗留 schema 中的业务表
- **AND** 系统 SHALL 输出执行前后表行数，证明脚本可重复执行且后续重复执行不会产生额外业务副作用
- **AND** 脚本不得触碰 GitLab 远端仓库、分支或 token 明文

#### Scenario: 拒绝非本地数据库清理

- **WHEN** 清理目标不是本地 `releasehub-postgres` 容器或 `release_hub` 数据库
- **THEN** 系统 SHALL 拒绝执行
- **AND** 操作者不得通过该脚本清理生产、CI 或未知数据库
