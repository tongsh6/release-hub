...
- 数据库：`application-local.yml` 连接 PostgreSQL（schema `release_hub`，`ddl-auto=update`，本地默认关闭 Flyway）；`application-test.yml`/`application-unitTest.yml` 使用 H2 PostgreSQL 模式并启用 Flyway；`application-prd.yml` 使用 PostgreSQL，Flyway 校验模式。
