...
class GroupDeleteTest {
    static class InMemoryPort implements GroupPort {
        ...
        @Override
        public void save(Group group) { byId.put(group.getId().value(), group); }
        @Override
        public Optional<Group> findById(GroupId id) { return Optional.ofNullable(byId.get(id.value())); }
        @Override
        public Optional<Group> findByCode(String code) {
            return byId.values().stream().filter(g -> g.getCode().equals(code)).findFirst();
        }
        @Override
        public List<Group> findAll() { return new ArrayList<>(byId.values()); }
        @Override
    ...
    void deleteByCode_ShouldFail_WhenHasChildren() {
        ...
        port.save(a); port.save(b); port.save(c);
