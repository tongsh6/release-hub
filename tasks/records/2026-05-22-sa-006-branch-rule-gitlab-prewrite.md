...
- 新增 `scripts/acceptance/sa006-branch-rule-gitlab-evidence.sh`，用独立 GitLab 项目、独立叶子分组、真实仓库引用和 PROJECT scoped BranchRule 采集可复现证据。
...
- `PASS=14 FAIL=0`
- 合规 NAMED 分支 `feature/sa006-allowed-20260522-153632` 写入成功，系统 version-info 记录一致，GitLab 直查为 `FOUND`。
- 不合规 NAMED 分支 `feature/sa006-denied-20260522-153632` 被写入前拒绝，迭代仓库集合未写入，GitLab 直查为 `NOT_FOUND`。
- 不合规 release 分支 `release/RW-20260522-22FE` 被创建前拒绝，GitLab 直查为 `NOT_FOUND`。
- Top10 静态扫描通过，报告：`.ai/reports/static-scan/20260522-233907/summary.md`。

## 剩余缺口

- 本轮补齐 PROJECT scoped 真实 GitLab 前置拒绝证据。
- SA-006 仍保留在 HEAD：下一步补 GLOBAL/SUB_PROJECT 组合的真实 GitLab 证据复核，并决定是否把聚焦脚本纳入默认全量验收入口。
